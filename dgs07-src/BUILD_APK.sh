...
"$tools_dir/apksigner" sign --ks "$project_dir/debug.keystore" --ks-key-alias dragon-debug --ks-pass pass:android --key-pass pass:android --out "$project_dir/DragonGameStudio-0.7-debug.apk" "$build_dir/aligned.apk"
"$tools_dir/apksigner" verify "$project_dir/DragonGameStudio-0.7-debug.apk"
echo "$project_dir/DragonGameStudio-0.7-debug.apk"