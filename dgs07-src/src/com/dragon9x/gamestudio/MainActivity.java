...
public final class MainActivity extends Activity {
 ...
 static final int BACKGROUND=11,PLAYER=12,ENEMY=13,IMPORT=14,EXPORT=15,TILES=16,CROP_SOURCE=17,EFFECTS=18,EFFECT_DESIGN_SOURCE=19,CHARACTER_DESIGN_SOURCE=20;

 ProjectStore store;
GameProject project;
Bitmap background,playerImage,enemyImage,tileImage,effectImage;
final HashMap<String,Bitmap> characterImages=new HashMap<>();
final HashMap<Integer,Bitmap> effectImages=new HashMap<>();
StageView editorStage,playStage;
TilePaletteView tilePalette;
FrameLayout pages;
LinearLayout projectPage,layoutPage,codePage,playPage,skillsPage,charactersPage,effectsPage,filesPage;
...
root.setBackgroundColor(0xffeef3f2);
  ...
  TextView header=text("DRAGON GAME STUDIO 0.7 • CHARACTER + MAP + SKILL",17,0xff087362);
...
charactersPage=new LinearLayout(this);
charactersPage.setOrientation(LinearLayout.VERTICAL);
ScrollView characterScroll=new ScrollView(this);
characterScroll.addView(charactersPage);
pages.addView(characterScroll);
...
projectPage.addView(button("Nhập dự án",v->pick(IMPORT,"application/zip")));
 ...
 void buildLayout(){
layoutPage.addView(text("Map 2D bản 0.7: kho tile nằm riêng ở bên trái, canvas map ở bên phải. Cắt tile xong sẽ lưu vào file tile-bank.png và xuất cùng dự án.",12,Color.DKGRAY));
...
imageRow.addView(button("Ảnh nhân vật cũ",v->pick(PLAYER,"image/png")));
...
filesPage.addView(text("/characters/ • frame nhân vật theo từng động tác",12,Color.DKGRAY));
for(CharacterAction a:project.characterActions)filesPage.addView(text("  /"+a.fileName+(store.hasCharacterAction(a.fileName)?" ✓":" (thiếu)")+" • "+a.name+" • "+a.frameCount+" frame",11,Color.DKGRAY));

  filesPage.addView(text("/engine/ • mã Java của xưởng, chỉ xem; đóng trong APK",12,Color.DKGRAY));
String[] sources={
"GameProject.java","StageView.java","GameRules.java","GameModule.java","Skill.java","SkillEffect.java","CharacterAction.java","TilePaletteView.java","ImageCropView.java","EffectPreviewView.java","AiClient.java","MainActivity.java"}
;
...
filesPage.addView(text("GSC 0.7 hỗ trợ if/then/else, and/or, HP/MP, vật phẩm, biến, trạng thái, trọng lực, nhảy/bay, chiến đấu và nhiều sự kiện mới. Bấm HƯỚNG DẪN ENGINE GSC để xem cú pháp đầy đủ.",12,Color.DKGRAY));
}
...
ScrollView scroll=new ScrollView(this);
scroll.addView(view);
new AlertDialog.Builder(this).setTitle("Hướng dẫn Engine GSC 0.7").setView(scroll).setPositiveButton("Đóng",null).show();
}
catch(Exception e){
error(e);
}
...
scroll.addView(box);
  ...
  AlertDialog dialog=new AlertDialog.Builder(this).setTitle("Cắt tile → kho riêng • 0.7").setView(scroll).setNegativeButton("Hủy",null).setPositiveButton("Lưu vào kho + chọn",null).create();
dialog.show();
dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->{
try{
...
skillsPage.addView(text("Danh sách skill 0.7. Mỗi skill có thể gán một hiệu ứng đã tạo ở tab Thiết kế skill; frame không còn bị khóa 16×16.",12,Color.DKGRAY));
...
for(int i=0;i<next.quickSkillIds.length;i++)if(next.quickSkillIds[i]==target.id)next.quickSkillIds[i]=0;
next.validate();
store.save(next);
project=next;
reloadImages();
...
if(charactersPage==null)return;
charactersPage.removeAllViews();
charactersPage.addView(text("TẠO NHÂN VẬT 0.7 • mỗi động tác có ảnh và số frame riêng. Ví dụ Đứng 2 frame, Chạy 4 frame; các động tác đánh, nhảy, bay và Skill 1–9 có thể đặt số frame hoàn toàn khác nhau.",12,Color.DKGRAY));
charactersPage.addView(button("Thiết lập nút điều khiển / skill nhanh",v->controlSetupDialog()));
for(int i=0;i<CharacterAction.KEYS.length;i++){
String key=CharacterAction.KEYS[i];
CharacterAction saved=project.characterAction(key);
CharacterAction view=saved==null?CharacterAction.create(key):saved;
LinearLayout card=new LinearLayout(this);
card.setOrientation(LinearLayout.VERTICAL);
boolean has=saved!=null&&store.hasCharacterAction(saved.fileName);
card.addView(text(CharacterAction.LABELS[i]+" • "+(saved==null?"chưa tạo":saved.frameCount+" frame • "+saved.frameWidth+"×"+saved.frameHeight+" • "+saved.frameMs+"ms"+(saved.loop?" • lặp":""))+(has?" ✓":""),11,0xff244237));
LinearLayout actions=new LinearLayout(this);
actions.addView(button(saved==null?"Nhập ảnh + tạo":"Thay ảnh",v->{
pendingCharacterKey=key;
pick(CHARACTER_DESIGN_SOURCE,"image/png");
}));
if(saved!=null){
actions.addView(button("Sửa / cắt frame",v->editCharacterAction(key)));
actions.addView(button("▶ Chạy frame",v->previewCharacterAction(saved)));
actions.addView(button("Xóa",v->deleteCharacterAction(saved)));
}
card.addView(actions);
charactersPage.addView(card);
}
}

 void editCharacterAction(String key){
CharacterAction action=project.characterAction(key);
if(action==null){
pendingCharacterKey=key;
pick(CHARACTER_DESIGN_SOURCE,"image/png");
return;
}
Bitmap bitmap=store.characterBitmap(action.fileName);
if(bitmap==null){
toast("Thiếu ảnh "+action.fileName+". Hãy bấm Thay ảnh.");
return;
}
characterDesignDialog(bitmap,action,key);
}

 void previewCharacterAction(CharacterAction action){
Bitmap bitmap=store.characterBitmap(action.fileName);
if(bitmap==null){
toast("Thiếu file động tác nhân vật");
return;
}
LinearLayout body=new LinearLayout(this);
body.setOrientation(LinearLayout.VERTICAL);
body.addView(text(action.toString(),12,Color.DKGRAY));
EffectPreviewView preview=new EffectPreviewView(this);
preview.setEffect(bitmap,action.frameWidth,action.frameHeight,action.frameCount,action.frameMs);
body.addView(preview,new LinearLayout.LayoutParams(-1,dp(300)));
AlertDialog dialog=new AlertDialog.Builder(this).setTitle("Chạy thử • "+action.name).setView(body).setNegativeButton("Đóng",null).setPositiveButton("Play / Pause",null).create();
...
dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->preview.toggle());
 ...
 void deleteCharacterAction(CharacterAction target){
new AlertDialog.Builder(this).setTitle("Xóa frame "+target.name+"?").setMessage("Game sẽ quay về ảnh nhân vật cũ player.png cho động tác này cho đến khi anh tạo lại.").setNegativeButton("Hủy",null).setPositiveButton("Xóa",(d,w)->{
try{
GameProject next=GameProject.fromJson(project.json().toString());
next.characterActions.removeIf(a->a.key.equals(target.key));
next.validate();
store.deleteCharacterAction(target);
store.save(next);
project=next;
reloadImages();
refreshCharacters();
 ...
 void characterDesignDialog(Bitmap source,CharacterAction original,String key){
if(source==null||!CharacterAction.allowedKey(key))return;
final Bitmap[] working={source.copy(Bitmap.Config.ARGB_8888,true)};
final ImageCropView[] cropRef=new ImageCropView[1];
CharacterAction base=original==null?CharacterAction.create(key):CharacterAction.copy(original);
if(original==null){
...
panel.addView(text("Động tác: "+base.name+". Kéo chọn vùng ảnh, cắt, xoay/lật, resize rồi đặt kích thước và số frame riêng cho động tác này.",11,Color.DKGRAY));
...
cropTools.addView(button("Toàn ảnh",v->cropRef[0].resetCrop()));
panel.addView(cropTools);

LinearLayout transform=new LinearLayout(this);
transform.addView(button("Xoay 90°",v->{working[0]=rotate90(working[0]);setCropBitmap(holder,cropRef,working[0]);}));
transform.addView(button("Lật ngang",v->{working[0]=flip(working[0],true);setCropBitmap(holder,cropRef,working[0]);}));
transform.addView(button("Lật dọc",v->{working[0]=flip(working[0],false);setCropBitmap(holder,cropRef,working[0]);}));
panel.addView(transform);

LinearLayout resize=new LinearLayout(this);
EditText outW=new EditText(this),outH=new EditText(this);
outW.setInputType(InputType.TYPE_CLASS_NUMBER);outH.setInputType(InputType.TYPE_CLASS_NUMBER);
outW.setText(""+working[0].getWidth());outH.setText(""+working[0].getHeight());
outW.setHint("Rộng px");outH.setHint("Cao px");
...
CheckBox loop=new CheckBox(this);
loop.setText("Lặp animation");
loop.setChecked(base.loop);
panel.addView(loop);
EffectPreviewView preview=new EffectPreviewView(this);
...
if(working[0].getWidth()%fw!=0||working[0].getHeight()%fh!=0)throw new IllegalArgumentException("Ảnh chưa chia đều cho frame "+fw+"×"+fh);
...
AlertDialog dialog=new AlertDialog.Builder(this).setTitle((original==null?"Tạo ":"Sửa ")+CharacterAction.labelOf(key)).setView(scroll).setNegativeButton("Hủy",null).setPositiveButton("Lưu động tác",null).create();
dialog.setOnDismissListener(d->preview.stop());
dialog.show();
dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(v->{
try{
CharacterAction a=CharacterAction.create(key);
a.name=CharacterAction.labelOf(key);
a.frameWidth=positive(frameW,"Rộng frame");
a.frameHeight=positive(frameH,"Cao frame");
a.frameCount=positive(frameCount,"Số frame");
a.frameMs=positive(frameMs,"Frame ms");
a.loop=loop.isChecked();
a.validate();
if(working[0].getWidth()%a.frameWidth!=0||working[0].getHeight()%a.frameHeight!=0)throw new IllegalArgumentException("Ảnh phải chia đều thành lưới frame "+a.frameWidth+"×"+a.frameHeight);
int cap=(working[0].getWidth()/a.frameWidth)*(working[0].getHeight()/a.frameHeight);
if(a.frameCount>cap)throw new IllegalArgumentException("Số frame tối đa là "+cap);
GameProject next=GameProject.fromJson(project.json().toString());
next.characterActions.removeIf(x->x.key.equals(key));
next.characterActions.add(a);
next.validate();
store.saveCharacterAction(a,TileLibrary.png(working[0]));
project=next;
store.save(project);
reloadImages();
refreshCharacters();
status.setText("Đã lưu "+a.name+" • "+a.frameCount+" frame");
dialog.dismiss();
}
catch(Exception e){error(e);}
});
...
a.add("Không gán skill");
int selected=0;
for(int i=0;i<project.skills.size();i++){
Skill skill=project.skills.get(i);a.add("#"+skill.id+" • "+skill.name);
...
try{
GameProject next=GameProject.fromJson(project.json().toString());
next.basicAction=basicKeys[basic.getSelectedItemPosition()];
for(int slot=0;slot<9;slot++){
...
effectsPage.addView(text("THIẾT KẾ SKILL / EFFECT 0.7 • nhập ảnh → cắt tự do → thu/phóng → xoay/lật → khai báo frame → chạy thử. Không còn giới hạn frame 16×16.",12,Color.DKGRAY));
...
error(ex);
 ...
 void buildCode(){
codePage.addView(text("GSC 0.7: sự kiện + hành động, có if/then/else, and/or, biến, HP/MP, vật phẩm và vật lý. AI tạo bản nháp để anh xem trước.",12,Color.DKGRAY));
...
row.addView(button("Thiết lập nút",v->controlSetupDialog()));
...
Bitmap b=store.characterBitmap(action.fileName);
if(b!=null)characterImages.put(action.key,b);
}
effectImages.clear();
for(SkillEffect effect:project.effects){
Bitmap b=store.effectBitmap(effect.fileName);
if(b!=null)effectImages.put(effect.id,b);
}
if(tilePalette!=null)tilePalette.setAtlas(tileImage);
editorStage.bind(project,background,playerImage,enemyImage,tileImage,effectImage,characterImages,effectImages,()->{
try{
store.save(project);
...
playStage.bind(project,background,playerImage,enemyImage,tileImage,effectImage,characterImages,effectImages,null);
}
...
((View)charactersPage.getParent()).setVisibility(page==5?View.VISIBLE:View.GONE);
((View)effectsPage.getParent()).setVisibility(page==6?View.VISIBLE:View.GONE);
((View)filesPage.getParent()).setVisibility(page==7?View.VISIBLE:View.GONE);

  if(page==4)refreshSkills();
if(page==5)refreshCharacters();
if(page==6)refreshEffects();
if(page==7)refreshFiles();
if(page==3){
try{
effectImage=activeEffectBitmap();
playStage.bind(project,background,playerImage,enemyImage,tileImage,effectImage,characterImages,effectImages,null);
...
cropDialog(source);
   ...
   else if(req==CHARACTER_DESIGN_SOURCE){
byte[] raw;
try(InputStream in=getContentResolver().openInputStream(uri)){
raw=ProjectStore.readStream(in,8*1024*1024);
}
Bitmap source=BitmapFactory.decodeByteArray(raw,0,raw.length);
if(source==null||(long)source.getWidth()*source.getHeight()>8000000)throw new IOException("Không đọc được ảnh nhân vật hoặc ảnh quá lớn");
String key=pendingCharacterKey;
pendingCharacterKey=null;
if(key==null||!CharacterAction.allowedKey(key))throw new IOException("Chưa chọn động tác nhân vật");
characterDesignDialog(source,project.characterAction(key),key);
}
