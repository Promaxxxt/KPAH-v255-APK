...
final HashMap<Integer,Long> skillLastUse=new HashMap<>();
Skill lastCastSkill;
String forcedAction="",animationKey="";
long forcedActionUntil,animationStarted;
boolean facingLeft;
String message="";
long messageUntil;
...
bind(p,bg,player,enemy,atlas,effect,null,null,change);
}

 void bind(GameProject p,Bitmap bg,Bitmap player,Bitmap enemy,Bitmap atlas,Bitmap effect,Map<String,Bitmap> characters,Map<Integer,Bitmap> effects,Runnable change){
project=p;
background=bg;
playerImage=player;
enemyImage=enemy;
...
if(characters!=null)characterImages.putAll(characters);
effectImages.clear();
if(effects!=null)effectImages.putAll(effects);
onChange=change;
cameraX=cameraY=0;
stop();
invalidate();
...
return;
 ...
 void attack(){
if(project==null)return;
useSkill(project.activeSkill(),project.basicAction);
}

 void useQuickSkill(int slot){
if(!playing||project==null||slot<0||slot>=project.quickSkillIds.length)return;
int id=project.quickSkillIds[slot];
if(id==0){
message="Nút S"+(slot+1)+" chưa gán skill";
messageUntil=System.currentTimeMillis()+900;
invalidate();
return;
}
Skill skill=project.skillById(id);
if(skill==null){
message="Skill #"+id+" không tồn tại";
messageUntil=System.currentTimeMillis()+900;
invalidate();
return;
}
...
if(!playing||health<=0||skill==null)return;
...
long now=System.currentTimeMillis();
Long used=skillLastUse.get(skill.id);
if(used!=null&&now-used<skill.cooldown)return;
skillLastUse.put(skill.id,now);
lastAttack=now;
lastCastSkill=skill;
triggerCharacterAction(characterAction,now);
effectX=playerX;
effectY=playerY;
...
invalidate();
 ...
 void triggerCharacterAction(String key,long now){
if(key==null||!CharacterAction.allowedKey(key))return;
CharacterAction action=project.characterAction(key);
long duration=action==null?350L:(long)action.frameCount*action.frameMs;
forcedAction=key;
forcedActionUntil=now+Math.max(120,duration);
animationKey=key;
...
if(x<-0.08f)facingLeft=true;
else if(x>0.08f)facingLeft=false;
...
if(!playing)drawActor(canvas,enemyImage,project.enemyX,project.enemyY,0xffbd4b44);
  ...
  drawPlayer(canvas,playing?playerX:project.playerX,playing?playerY+airOffset:project.playerY,false);
drawEffect(canvas);
...
c.drawRect(dest,paint);
  ...
  drawPlayer(c,playing?playerX:project.playerX,playing?playerY+airOffset:project.playerY,true);
drawEffect(c);
c.restore();
...
c.drawBitmap(bitmap,null,new RectF(x-w/2,y-h/2,x+w/2,y+h/2),paint);
 ...
 String currentCharacterAction(){
if(!playing)return "stand";
long now=System.currentTimeMillis();
if(now<forcedActionUntil&&CharacterAction.allowedKey(forcedAction))return forcedAction;
if(flying)return "fly";
if(airOffset<0||verticalSpeed!=0)return "jump";
if(Math.hypot(axisX,axisY)>.08)return "run";
return "stand";
}

 void drawPlayer(Canvas c,float x,float y,boolean flat){
String key=currentCharacterAction();
long now=System.currentTimeMillis();
if(!key.equals(animationKey)){
animationKey=key;
animationStarted=now;
}
CharacterAction action=project.characterAction(key);
Bitmap bitmap=action==null?null:characterImages.get(key);
if(action==null||bitmap==null){
if(flat)drawFlatActor(c,playerImage,x,y,0xff53bbdb);
else drawActor(c,playerImage,x,y,0xff53bbdb);
return;
}
int fw=action.frameWidth,fh=action.frameHeight,cols=bitmap.getWidth()/fw,rows=bitmap.getHeight()/fh;
int capacity=Math.min(action.frameCount,Math.max(1,cols*rows));
long elapsed=Math.max(0,now-animationStarted);
int frame=(int)(elapsed/action.frameMs);
if(action.loop)frame%=capacity;
else frame=Math.min(capacity-1,frame);
int sx=(frame%cols)*fw,sy=(frame/cols)*fh;
float max=flat?64f:128f;
float scale=Math.min(1f,max/Math.max(fw,fh));
float w=fw*scale,h=fh*scale;
paint.setColor(Color.WHITE);
c.save();
if(facingLeft)c.scale(-1,1,x,y);
c.drawBitmap(bitmap,new Rect(sx,sy,sx+fw,sy+fh),new RectF(x-w/2,y-h/2,x+w/2,y+h/2),paint);
c.restore();
}

 void drawQuickButtons(Canvas c,boolean flat){
for(int slot=0;slot<9;slot++){
float x=quickX(slot,flat),y=quickY(slot,flat),r=flat?12:22;
int id=project.quickSkillIds[slot];
paint.setColor(id==0?0x554f5963:0xaa3f8fb5);
c.drawCircle(x,y,r,paint);
paint.setColor(Color.WHITE);
paint.setTypeface(Typeface.DEFAULT_BOLD);
paint.setTextSize(flat?8:13);
String label="S"+(slot+1);
float tw=paint.measureText(label);
c.drawText(label,x-tw/2,y+(flat?3:4),paint);
}
}

 float quickX(int slot,boolean flat){
if(flat)return 312+(slot%3)*29;
return project.attackX-110+(slot%3)*55;
}

 float quickY(int slot,boolean flat){
if(flat)return 82+(slot/3)*29;
return project.attackY-160+(slot/3)*55;
}

 int quickHit(float x,float y,boolean flat){
float r=flat?16:30;
for(int slot=0;slot<9;slot++)if(Math.hypot(x-quickX(slot,flat),y-quickY(slot,flat))<r)return slot;
return -1;
}

 void drawEffect(Canvas c){
if(!playing||lastAttack==0||lastCastSkill==null)return;
Skill skill=lastCastSkill;
SkillEffect effect=project.effectById(skill.effectId);
Bitmap castEffect=effect==null?null:effectImages.get(effect.id);
if(castEffect==null)castEffect=effectImage;
long elapsed=System.currentTimeMillis()-lastAttack;
if(effect!=null&&castEffect!=null){
long duration=skill.frameMs*(long)effect.frameCount;
if(elapsed<0||elapsed>=duration)return;
int frame=(int)(elapsed/skill.frameMs);
int fw=effect.frameWidth,fh=effect.frameHeight,columns=castEffect.getWidth()/fw,rows=castEffect.getHeight()/fh;
if(columns>0&&rows>0&&frame<Math.min(effect.frameCount,columns*rows)){
int sx=(frame%columns)*fw,sy=(frame/columns)*fh;
paint.setColor(Color.WHITE);
c.drawBitmap(castEffect,new Rect(sx,sy,sx+fw,sy+fh),new RectF(effectX-fw/2f,effectY-fh/2f,effectX+fw/2f,effectY+fh/2f),paint);
return;
}
}
...
if(castEffect!=null&&castEffect.getWidth()%16==0&&castEffect.getHeight()%16==0){
int columns=castEffect.getWidth()/16;
if(frame<columns*(castEffect.getHeight()/16)){
paint.setColor(Color.WHITE);
c.drawBitmap(castEffect,new Rect((frame%columns)*16,(frame/columns)*16,(frame%columns+1)*16,(frame/columns+1)*16),new RectF(effectX-16,effectY-16,effectX+16,effectY+16),paint);
return;
}
}
...
quick=quickHit(x,y,true);
if(quick>=0)useQuickSkill(quick);
}
if(quick<0&&Math.hypot(x-project.attackX,y-project.attackY)<38&&(action!=MotionEvent.ACTION_MOVE||project.attackMode.equals("Giữ")))attack();
}
...
int quick=-1;
if(action==MotionEvent.ACTION_DOWN||action==MotionEvent.ACTION_POINTER_DOWN){
quick=quickHit(x,y,false);
if(quick>=0)useQuickSkill(quick);
}
if(quick<0&&Math.hypot(x-project.attackX,y-project.attackY)<72&&(action!=MotionEvent.ACTION_MOVE||project.attackMode.equals("Giữ")))attack();
}
if(!move)axisX=axisY=0;
invalidate();
return true;
